...
#include "wong1109_lab5/visualization.h"
...
public:
  ScanProcessor(ros::NodeHandle &n) : curr_trans(Transform()) {
    ...
    corr_viz =
        new CorrespondenceVisualizer(marker_pub, "scan_match", FRAME_POINTS);
  ...
  void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr &msg) {
    ...
    while (count < MAX_ITER && (curr_trans != prev_trans || count == 0)) {
      ...
      //************************************************ Find correspondence
      //between points of the current and previous frames  *************** ////
      // ****************************************************
      // getCorrespondence() function is the fast search function and
      // getNaiveCorrespondence function is the naive search option **** ////

      // getCorrespondence(prev_points, transformed_points, points, jump_table,
      // corresponds, A*count*count+MIN_INFO);

      getNaiveCorrespondence(prev_points, transformed_points, points,
                             jump_table, corresponds,
  ...
  void publishPos() {
    ...
    tf::Matrix3x3 tf3d;
    tf3d.setValue(static_cast<double>(global_tf(0, 0)),
                  static_cast<double>(global_tf(0, 1)), 0,
                  static_cast<double>(global_tf(1, 0)),
                  static_cast<double>(global_tf(1, 1)), 0, 0, 0, 1);
    ...
    tr.setRotation(q);
    br.sendTransform(
        tf::StampedTransform(tr, ros::Time::now(), "map", "laser"));
  }
...
int main(int argc, char **argv) {
  ...
  // SUBSCRIBE
  ros::Subscriber sub =
      n.subscribe(TOPIC_SCAN, 1, &ScanProcessor::handleLaserScan, &processor);
